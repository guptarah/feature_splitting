...
def get_accuracy(w,X,targets,sample_wts):
	...
	return probability,obt_classes,accuracy,equals

def get_wtd_accuracy(w,X,targets,sample_wts):
        w = numpy.matrix(w)
        X = numpy.matrix(X)
        mat_exp_w_tran_x = numpy.exp(X*w)
        normalizing_factor = numpy.sum(mat_exp_w_tran_x,axis=1)
        ...
        equals = (obt_classes==targets)*1
        accuracy = (float)(numpy.sum(numpy.multiply(equals,sample_wts)))/len(numpy.multiply(equals,sample_wts)) # the accuracy
...
def sim_anneal(w,X,targets,eta,one_in_k_enc,cur_feat_indic,sim_anl_prob,sample_wts):
	...
	probability,obt_classes,accuracy1,equals1 = get_wtd_accuracy(cur_wts_new1,cur_data1,targets,sample_wts)
	...
	probability,obt_classes,accuracy2,equals2 = get_wtd_accuracy(cur_wts_new2,cur_data2,targets,sample_wts)

	print >> f1, "accuracy1:", accuracy1, "accuracy2:", accuracy2
	
	if (accuracy1>=accuracy2):
               print >> f1, "---sim anneal iter end---\n"
               return cur_feat_indic,cur_wts_new1,equals1
       	else:
               print >> f1,"modified feat_indic with ", numpy.sum(new_feat_indic), "features"
               print >> f1,"------- sim anneal iter end ------------\n"
               return new_feat_indic,cur_wts_new2,equals2
...
for clasf_iter in range(0,num_classifiers):
	...
	for conv_iters in range(0,maxiter):
		...
		if (conv_iters <= -1 ):
			...
			probability,obt_classes,accuracy,equals = get_accuracy(w_new,X,train_targets,sample_wts)
			...
			new_cur_feat_indic_sa, new_wt_cur_clasf_sa,equals = sim_anneal(w,X,train_targets,eta,one_in_k_enc,cur_feat_indic,sim_anl_prob,sample_wts)
			...
			probability,obt_classes,accuracy,equals = get_accuracy(new_wt_cur_clasf_sa,data_cur_classifier,train_targets,sample_wts)
			data_acc_iters[clasf_iter,conv_iters] = accuracy

			print >> f1,"iter: ",clasf_iter, "accuracy: ",accuracy
			...
			if (max_acc_pos < (conv_iters-10)):
				...
				probability,obt_classes,accuracy,equals = get_accuracy(final_wts_ds,data_cur_classifier,train_targets,sample_wts)
	...
	# get cumulative probability over classifier trained till now
		...
		data_cur_clasf = X[:,(cur_feat_indic==1)]
                probability_train,obt_classes,indiv_acc_train,equals = get_accuracy(w_cur_clasf,data_cur_clasf,train_targets,sample_wts)
...
for clasf_iter in range(0,num_classifiers):
	...
	probability_train,obt_classes,indiv_acc_train,equals = get_accuracy(wts_ds,X_train_ds,train_targets,numpy.ones((X.shape[0],1)))

	X_test_ds = X_test[:,(feat_indic==1)]
	probability_test,obt_classes,indiv_acc_test,equals_dev = get_accuracy(wts_ds,X_test_ds,test_targets,numpy.ones((X_test.shape[0],1)))

	X_dev_ds = X_dev[:,(feat_indic==1)]
	probability_dev,obt_classes,indiv_acc_dev,equals_test = get_accuracy(wts_ds,X_dev_ds,dev_targets,numpy.ones((X_dev.shape[0],1)))

	if clasf_iter == 0:
		cum_train_prob = indiv_acc_train*numpy.log(probability_train)
		cum_test_prob = indiv_acc_train*numpy.log(probability_test)
		cum_dev_prob = indiv_acc_train*numpy.log(probability_dev)
		oracle_test_fusion = equals_test
		print (float)(numpy.sum(oracle_test_fusion))/len(oracle_test_fusion)
	else:
		cum_train_prob += indiv_acc_train*numpy.log(probability_train)
		cum_test_prob += indiv_acc_train*numpy.log(probability_test)
		cum_dev_prob += indiv_acc_train*numpy.log(probability_dev)
		oracle_test_fusion = numpy.logical_or(oracle_test_fusion,equals_test)
		print (float)(numpy.sum(oracle_test_fusion))/len(oracle_test_fusion) 
...
# get the oracle test accuracy
oracle_accuracy = (float)(numpy.sum(oracle_test_fusion))/len(oracle_test_fusion) # the accuracy
...
if sim_anl_prob > 1:
	...
	result_file_name = dir_name + '/result_file_wtd'
	f3 = open(result_file_name,'a+')
	print >> f3, oracle_accuracy,best_cum_acc_o, best_indiv_acc_o, best_cum_acc, best_indiv_acc, first_clsf_acc

else:
	result_file_name = dir_name + '/result_file_sa_wtd'
        f3 = open(result_file_name,'a+')
        print >> f3, oracle_accuracy,best_cum_acc_o,best_indiv_acc_o,best_cum_acc, best_indiv_acc, first_clsf_acc
