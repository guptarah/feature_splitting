...
def sim_anneal(w,X,targets,eta,one_in_k_enc,cur_feat_indic,sim_anl_prob,sample_wts,X_dev,dev_targets):
	...
	cur_data_dev1 = X_dev[:,(cur_feat_indic==1)]
	sample_wts_dev = numpy.ones((X_dev.shape[0],1))
	probability,obt_classes,accuracy1,wtd_ll1 = get_accuracy(cur_wts_new1,cur_data_dev1,dev_targets,sample_wts_dev)
	...
	cur_data_dev2 = X_dev[:,(new_feat_indic==1)]
	probability,obt_classes,accuracy2,wtd_ll2 = get_accuracy(cur_wts_new2,cur_data_dev2,dev_targets,sample_wts_dev)
...
#	f_cv = open('./cv_cur_dev','w+')
#	pickle.dump(cv_indices,f_cv)
#	f_cv.close()
#else:
#	# split into 90% train and 10% test
#	f_cv = open('./cv_cur_dev','r+')
...
if sim_anl_prob > 1:
	log_file_name = dir_name + '/logfile_dev_baseline'
	f1 = open(log_file_name,'w+')
else:
	log_file_name = dir_name + '/logfile_dev_sa'
	f1 = open(log_file_name,'w+')


if sim_anl_prob > 1:
	f_w=open(dir_name+'weight_file', 'w+')  # use the same initialization as the optimization on train set
	pickle.dump(clasf_wts,f_w)
	f_w.close()
else:
	f2=open(dir_name+'new_wts_dev', 'r+')
...
for clasf_iter in range(0,num_classifiers):
	...
	for conv_iters in range(0,maxiter):
		...
		if (conv_iters <= -1 ):
			...
			print "This loop should never run"
			...
			new_cur_feat_indic_sa, new_wt_cur_clasf_sa,wtd_data_ll = sim_anneal(w,X,train_targets,eta,one_in_k_enc,cur_feat_indic,sim_anl_prob,sample_wts,X_dev,dev_targets)
			w = update_wt_matrix(new_wt_cur_clasf_sa,w,new_cur_feat_indic_sa)
			cur_feat_indic = new_cur_feat_indic_sa		

			data_cur_classifier = X_dev[:,(cur_feat_indic == 1)]
			new_wt_cur_clasf_sa = w[(cur_feat_indic == 1),:]
			sample_wts_dev = numpy.ones((X_dev.shape[0],1))	
			probability,obt_classes,accuracy,wtd_data_ll = get_accuracy(new_wt_cur_clasf_sa,data_cur_classifier,dev_targets,sample_wts_dev)
...
#		final_feat_indics = store_feat_indic[max_ll_pos]
			...
			if (max_acc_pos < (conv_iters-10)):
				...
				data_cur_classifier = X_dev[:,(final_feat_indics == 1)]
				final_wts_ds = final_wts[(final_feat_indics == 1),:]
				sample_wts_dev = numpy.ones((X_dev.shape[0],1))
				probability,obt_classes,accuracy,wtd_data_ll = get_accuracy(final_wts_ds,data_cur_classifier,dev_targets,sample_wts_dev)
...
if sim_anl_prob > 1:
	f_new_wts = open(dir_name+'new_wts_dev','w+')
	pickle.dump(clasf_wts,f_new_wts)
	f_new_wts.close()

	result_file_dev_name = dir_name + '/result_file_dev'
	f3 = open(result_file_dev_name,'a+')
	print >> f3, best_cum_acc_o, best_indiv_acc_o, best_cum_acc, best_indiv_acc, first_clsf_acc

else:
	result_file_dev_name = dir_name + '/result_file_dev_sa'
        f3 = open(result_file_dev_name,'a+')
        print >> f3, best_cum_acc_o,best_indiv_acc_o,best_cum_acc, best_indiv_acc, first_clsf_acc
